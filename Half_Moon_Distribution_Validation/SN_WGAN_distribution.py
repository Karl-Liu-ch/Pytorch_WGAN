...
class Discriminator(nn.Module):
    def __init__(self, input_nums):
        ...
        def Linear(input_nums, output_nums):
            layer = []
            layer.append(nn.utils.spectral_norm(nn.Linear(input_nums, output_nums)))
            layer.append(nn.LeakyReLU())
            return layer
...
class WGAN():
    ...
    def save(self):
        torch.save(self.G.state_dict(), "../checkpoint/SN_WGAN_distribution/G_distribution.pth")
        torch.save(self.D.state_dict(), "../checkpoint/SN_WGAN_distribution/D_distribution.pth")
        print("model saved!")

    def load(self):
        self.G.load_state_dict(torch.load("../checkpoint/SN_WGAN_distribution/G_distribution.pth"))
        self.D.load_state_dict(torch.load("../checkpoint/SN_WGAN_distribution/D_distribution.pth"))
        print("load G and D!")
...
if __name__ == '__main__':
    try:
        os.mkdir('../checkpoint/SN_WGAN_distribution/')
    except:
        pass
    WGAN = WGAN()
    WGAN.train()
